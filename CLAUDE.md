...
- `symfony/README.md` + `symfony/claude_guide_symfony.pdf` — Doc Symfony (projet Synoptic), **fichier unique organisé en sections** (pas un dossier multi-fichiers comme react/backups/bvp) pour garder une seule ligne dans la sidebar ; 1ère section (09/07/2026) : cache et environnements multi-tenant (Symfony Runtime / Doctrine `when@` / Twig auto_reload / piège APP_DEBUG en CLI / nom de classe container incluant le flag debug) ; 2ème section ajoutée le 20/07/2026 : correctif pérenne d'`APP_DEBUG` (remplace le contournement CLI manuel de la 1ère section, barré mais conservé car toujours d'actualité sur BVP/Épione non adaptés) + profiler Symfony actif en prod indépendamment du debug (`PROFILER_COLLECT`) + état des lieux BVP/Épione (même piège, pas encore corrigé).
- `serveurs/serveur-hds-mylbe.md` + `serveurs/claude_guide_serveur_hds_mylbe.pdf` — Doc infra serveur HDS Mylbe (projet Épione/ISIS) : résolution de l'incident `.!!XXXX` pendant `cache:clear` (root cause dans `Filesystem::remove()` de Symfony), résolution du blocage `apt update` (clé GPG sury.org expirée), audit sécurité complet (Debian 11 EOL LTS le 2026-08-31, `debsecan`, paquets à mettre à jour), et plan de mise à jour du serveur en 3 séances (risques, fenêtre de maintenance, points en attente avec les partenaires José/Michaël). Nouveau dossier thème `serveurs/` créé pour ce document, pensé pour accueillir la doc d'autres serveurs à l'avenir.
